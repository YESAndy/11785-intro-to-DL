...
class RNN_Phoneme_Classifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            # hidden = np.zeros((self.num_layers, batch_size, self.hidden_size), dtype=float)
            z = self.x[:, t, :].reshape(batch_size, self.input_size)
            for layer in range(self.num_layers):
                # Run the rnn cell with the correct parameters and update
                z = self.rnn[layer].forward(z, self.hiddens[t][layer])
    ...
    def backward(self, delta):
        ...
        * Iterate in reverse order of time (from seq_len-1 to 0)
            * Iterate in reverse order of layers (from num_layers-1 to 0)
                ...
                * If you aren't at the first layer, you will want to add
                  dx to the gradient from layer-1th layer.
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len, 0, -1):
            dz, dh[-1] = self.rnn[-1].backward(dh[-1], self.hiddens[t][-1, :, :], self.hiddens[t][-2, :, :],
                                               self.hiddens[t - 1][-1, :, :])
            if self.num_layers > 2:
                for layer in range(self.num_layers - 2, 0, -1):
                    # dh(t, l - 1), dh(t - 1, l) < - dh(t, l)
                    dh[layer - 1] += dz
                    dz, dh[layer] = self.rnn[layer].backward(dh[layer], self.hiddens[t][layer, :, :],
                                                             self.hiddens[t][layer - 1, :, :],
                                                             self.hiddens[t - 1][layer, :, :])
            dh[0] += dz
            dz, dh[0] = self.rnn[0].backward(dh[0], self.hiddens[t][0, :, :],
                                             self.x[:, t-1, :].reshape(batch_size, self.input_size),
                                             self.hiddens[t - 1][0, :, :])

            print(t)
        #     dhnextL, dh[-1] = self.rnn[-1].backward(dh[-1], self.hiddens[t][-1, :, :], self.hiddens[t][-2, :, :],
        #                                             self.hiddens[t - 1][-1, :, :])
        #     dhnextL += dh[-2]
        #     if self.num_layers > 2:
        #         for layer in range(self.num_layers - 2, 0, -1):
        #             #  dh(t,l-1), dh(t-1,l) <- dh(t, l)
        #
        #             dhnextL, dh_temp = self.rnn[layer].backward(dhnextL, self.hiddens[t][layer, :, :],
        #                                                         self.hiddens[t][layer - 1, :, :],
        #                                                         self.hiddens[t - 1][layer, :, :])
        #             # update dh(t-1,l)
        #             dh[layer] += dh_temp.copy()
        #
        #     _, dh_temp = self.rnn[0].backward(dh[0], self.hiddens[t][0, :, :],
        #                                       self.x[:, t, :].reshape(batch_size, self.input_size),
        #                                       self.hiddens[t - 1][0, :, :])
        #     dh[0] += dh_temp.copy()
        dh_0 = dh / batch_size
        print("my dh is ", dh_0[0][0])
        return dh_0
