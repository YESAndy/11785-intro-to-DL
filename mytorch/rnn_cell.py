...
class RNN_Cell(object):
    ...
    def forward(self, x, h):
        ...
        h_prime = self.activation(
            np.einsum('ijk,jk->ij', x.reshape(x.shape[0], 1, x.shape[1]), self.W_ih)
            + np.einsum('ijk,jk->ij', h.reshape(h.shape[0], 1, h.shape[1]), self.W_hh)
            + self.b_ih + self.b_hh)
    ...
    def backward(self, delta, h, h_prev_l, h_prev_t):
        ...
        self.dW_ih += np.average(
            np.einsum('ijk,ijk->ijk', dz.reshape(batch_size, self.hidden_size, 1),
                      h_prev_l.reshape(batch_size, 1, self.input_size)),
            axis=0)
        self.dW_hh += np.average(
            np.einsum('ijk,ijk->ijk', dz.reshape(batch_size, self.hidden_size, 1),
                      h_prev_t.reshape(batch_size, 1, self.hidden_size)),
            axis=0)
        self.db_ih += np.average(dz, axis=0)
        self.db_hh += np.average(dz, axis=0)

        # 2) Compute dx, dh
        dx = np.einsum('ijk,kj->ij', dz.reshape(batch_size, 1, self.hidden_size), self.W_ih)
        dh = np.einsum('ijk,kj->ij', dz.reshape(batch_size, 1, self.hidden_size), self.W_hh)
